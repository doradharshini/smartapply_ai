...
Exposes government scheme data as MCP tools for ADK agents.
"""
...
def get_scheme_by_keyword(keyword: str) -> dict:
    ...
    for item in data:
        if (
            lookup_key in item.get("name", "").lower()
            or lookup_key in item.get("description", "").lower()
            or lookup_key in item.get("eligibility", "").lower()
        ):
